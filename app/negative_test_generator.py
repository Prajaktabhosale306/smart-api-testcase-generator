...
class NegativeTestGenerator:
    ...
    def generate_negative_tests(self, method, method_details, path_params=None, query_params=None):
        ...
        if path_params:
            negative_tests.extend(self._generate_missing_path_params(path_params))

        # Handle query parameters
        if query_params:
            negative_tests.extend(self._generate_missing_query_params(query_params))
    ...
    def _generate_missing_path_params(self, path_params):
        return [{
            "type": "missing_path_param",
            "error_field": param["name"]
        } for param in path_params]

    def _generate_missing_query_params(self, query_params):
        return [{
